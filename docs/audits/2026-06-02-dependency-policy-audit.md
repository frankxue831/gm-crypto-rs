...
Generated by a read-only Claude Code dynamic workflow, with orchestrator recon of the three manifests + `deny.toml`. Agents ran
read-only `cargo tree`/`metadata` (transient gitignored `Cargo.lock` only). No files edited, nothing
committed/pushed/published/tagged, no deps changed, no CI touched.
