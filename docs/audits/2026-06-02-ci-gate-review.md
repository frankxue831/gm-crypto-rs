...
Generated by a read-only Claude Code dynamic workflow, with orchestrator re-verification of the line-level claims against `ci.yml`,
`deny.toml`, the `gmcrypto-simd` test cfg-gating, and the `Error` enum. No files edited, nothing
committed/pushed/published/tagged, no CI or settings touched.
