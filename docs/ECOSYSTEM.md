...
**Verification in `gmcrypto-envelope-lite`:** its sources must contain no re-export of `gmcrypto-core`, its public-API test must pass, and its public-API snapshot must show no unreviewed core types. This charter states the obligation; the commands that discharge it live in that repository, because a public charter has no business publishing a private repository's file layout (`docs/disclosure-boundary.md` §2.2). Applying the same encapsulation rule to a future crate is policy-only until that crate adds an equivalent gate.
...
**Verification in `gmcrypto-envelope-lite`:** run that repository's full open-source-boundary suite — its boundary test, its worktree boundary scan, and its Cargo-package scan — and require all three to pass before any publication step. Approval of a fresh export, repository migration, or history rewrite is a policy-only human gate.
...
The canonical recipe is a single gate script maintained in the envelope repository, taking the core candidate's path and an output path for the evidence table. Run it rather than reconstructing the commands by hand: it exports a clean envelope tree and the candidate core side by side into a disposable directory, runs both phases below, and fails closed. That repository also exposes the same script under a `workflow_dispatch` trigger, taking the candidate ref as input and uploading the evidence table as an artifact.

The automation lives downstream rather than here, and the reason is worth stating because it generalizes: the envelope repository is private and this one is public, so a workflow *there* can read this repository at any ref with no credentials at all, whereas the inverse would mean storing a private-repository token in a public repository's secrets. When only one direction needs a credential, put the automation on the side that needs none.

**Every phase runs in every feature configuration the downstream crate ships.** For `gmcrypto-envelope-lite` that is the default build and `--features aead`. This is not a formality: the `aead` feature enables `gmcrypto-core/sm4-aead`, which is defined as `["dep:gmcrypto-simd"]` and so pulls `gmcrypto-simd` — and, on x86_64 and aarch64, its target-gated `cpufeatures` detection dependency — into the compiled graph. A default-features-only gate never reaches that core surface at all.
...
**Phase 2 — candidate injected.** Inject the core release candidate through a Cargo `[patch]` override. **The override path must be relative.** An absolute path writes a developer home directory into `Cargo.toml` — exactly the disclosure class `docs/disclosure-boundary.md` §2.1 forbids, and exactly what the downstream boundary scan exists to reject. Export the candidate beside the envelope and point at it relatively — `../core/crates/gmcrypto-core`. Run the Cargo behavioral operations without `--locked` so Cargo may rewrite only the disposable lockfile; the committed downstream lockfile remains untouched. Keep the formatting and boundary commands unchanged — they are what proves no absolute path survived. If the candidate version falls outside the downstream version requirement — a major-version candidate against a caret requirement, or any candidate outside an exact pin — adjust that requirement in the temporary gate copy only, never in the real downstream checkout; without it the patch does not resolve and the candidate is never tested. Against the patched candidate, run Clippy, unit tests, documentation tests, and every dynamically discovered behavioral integration target. `release_documents` is the sole deliberate post-patch exclusion, because it validates registry metadata rather than runtime or API compatibility and a path package has no registry checksum. A breaking result requires a documented migration note before the core release ships.
...
**Verification:** preserve the evidence table and the exact tested core and envelope commits as release evidence. The script fails closed — non-zero exit, and no evidence table written — on any step failure, so an evidence table the script produced records a passing run. That guarantee covers script output only; a hand-written evidence file is only as trustworthy as the process behind it. The gate script carries its own self-test in that repository, on the same reasoning as `check_disclosure_boundary.py --self-test` here: a gate that has silently stopped checking turns a green result into false assurance. The trigger remains manual by design — a maintainer runs it as a step in the core release runbook. Automatic cross-repository triggering is deferred until repeated maintenance justifies it.
