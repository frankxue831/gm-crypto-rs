...
# One-time: rustup toolchain install nightly && cargo install cargo-fuzz --version 0.13.2 --locked
...
Located at `crates/gmcrypto-core/benches/timing_leaks.rs`. **Sixteen
targets at the default / `sm4-bitsliced` budget; eighteen under
`sm4-bitsliced-simd`; twenty-one under `sm4-bitsliced-simd,sm4-aead`;
twenty-two under `sm4-bitsliced-simd,sm4-aead,sm4-xts`; twenty-three under
`sm4-bitsliced-simd,sm4-aead,sm4-xts,sm2-key-exchange`; twenty-four under
...
the production-surface count stayed twenty-three. v1.9 (the TLCP toolkit C FFI) also adds NO dudect
...
public-inputs), so the production-surface count stayed twenty-three. v1.11 (the `aead` 0.6 trait
...
guarded by the differential `fuzz_sm4_aead_traits`, NOT by prose. The
2026-08-10 telemetry-only noise twin then brings the current harness total to
twenty-four without adding a production timing claim.** (v0.3 added
...
2026-08-10 added the base `noise_twin_class_split` required-telemetry target,
so the base count is now 16. Its value does NOT gate pending calibration.)
...
sentinel @0.55 (nightly); the probes are kept as telemetry. The 2026-08-10
`noise_twin_class_split` target implements the class-split-aware revisit door
as required non-blocking telemetry; it is not an authoritative relative gate
until hosted-runner calibration + injected-leak controls pass (Q19.5 → v0.20
§5/§6; `docs/v0.5-dudect-recalibration.md`).
...
| `noise_floor_fp_invert` | **telemetry only** (non-blocking `REL-TELEMETRY`) | v0.19 — fix-vs-fix `Fp::invert` noise-floor probe; same construction over `Fp`. Measures ~0.005 even when `ct_fp_invert` spikes to 0.26 — the proof that the runner noise is in the two-input class split, not the operation. Historical contrast for the current class-split-aware reference. |
| `noise_twin_class_split` | **required measurement; non-blocking value** (`NOISE-TWIN`) | 2026-08-10 — two distinct fixed 32-byte values at distinct addresses feed one fixed-work XOR/rotate body with no data-dependent branch, lookup, allocation, or loop bound. Completeness-gated in every PR/nightly run, but not duration-matched to inversion and not part of any relative threshold until multi-night calibration + injected-leak controls pass. |
...
crates/gmcrypto-core/
  ...
  benches/timing_leaks.rs   # dudect harness — 24 targets total: 20 ct_* leak targets + negative control + 3 telemetry probes
...
crates/gmcrypto-c/          # v0.4 W4 — C ABI shim (cdylib + staticlib + rlib)
  ...
  examples/sm4_gcm_streaming.c # v0.10 — chunked SM4-GCM streaming AEAD round-trip via the C ABI (CI syntax-checks; link/run remains local)
  examples/sm4_ccm.c        # #120 — single-shot SM4-CCM: encrypt w/ AAD -> print the ct||tag layout -> decrypt round-trip -> three REJECTION cases (tampered tag, tampered ciphertext, modified AAD). CCM packs ct||tag into ONE buffer (no separate tag out-param, unlike GCM) and decrypt takes that combined buffer + the SAME tag_len. CI syntax-checks; link/run remains local
  examples/sm4_xts_sector.c # v0.13 — 512-byte SM4-XTS sector encrypt/decrypt round-trip via the C ABI (sector# as tweak; CI syntax-checks)
  examples/sm4_xts_multisector.c # v0.16 — in-place 8-sector ("disk region") SM4-XTS round-trip via the C ABI (start_sector: u64, auto-incrementing tweak; CI syntax-checks)
  examples/sm2_key_exchange.c # v1.2 — full two-party GM/T 0003.3 handshake via the C ABI (both tags verified, keys agree; CI syntax-checks, link/run remains local)
  examples/x509_verify.c    # v1.4 — leaf-vs-issuer certificate signature verification via the C ABI (argv DER paths; prints serial + validity, never checks a clock; CI syntax-checks, link/run remains local)
  examples/tlcp_handshake.c # v1.9 — full TLCP handshake-to-record flow via the C ABI: no-confirm SM2-KX → derive_master_secret/derive_key_block → SM4-CBC record protect/deprotect round-trip → Finished verify_data (two embedded static keypairs; CI syntax-checks, link/run remains local)
  examples/tlcp_verify_pair.c # v1.9 — TLCP [sign,enc] certificate-pair verification via the C ABI (argv DER paths → verify_pair; loudly NOT endpoint auth; a real gmssl pair verifies end-to-end; CI syntax-checks, link/run remains local)
...
- **The v0.19 fix-vs-fix relative gate was FALSIFIED — don't re-add it.** v0.19
  ...
  `REL-TELEMETRY`. The current `noise_twin_class_split` reference is the
  required class-split-aware telemetry (two *different* inputs through a
  known-CT op), not a same-input probe; do not turn it into a relative gate
  until multi-night calibration and injected-leak controls pass. See
