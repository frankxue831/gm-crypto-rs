...
v0.23 = pre-1.0 re-audit remediation (per `docs/v0.23-scope.md` Q23.1–Q23.9 + `docs/v1.0-reaudit.md`). A multi-model adversarial pre-publish re-audit (Codex gpt-5.5 + Grok, each source-verified) returned NO-GO as-is — core primitives sound, but 2 API/ABI BLOCKERs + crypto/zeroize/doc should-fixes. **W1 (API):** `Sm2PrivateKey::public_key() -> Sm2PublicKey` (was `ProjectivePoint`); doc-hide the raw EC point surface (`sm2::point::ProjectivePoint` type+module+re-export, bare `add`/`double`/`neg`/generator/identity, `Sm2PublicKey::{from_point,point}`+`From<ProjectivePoint>`) + `asn1::{reader,writer,oid}` modules + `traits::{Hash,Mac,BlockCipher}` (all kept `pub` for in-repo dev — Group-A pattern; wire types `asn1::{encode,decode}_sig`+`Sm2Ciphertext` stay public); `spki::{encode,decode}`+`sec1::EcPrivateKey.public` speak `Sm2PublicKey`; byte output unchanged. **W2 (crypto):** single-shot `mode_gcm::{encrypt,encrypt_with_tag_len}` fallible (reject `> 2^36−32` B); SM2 sign/encrypt take the fallible `rand_core::TryCryptoRng` bound (RNG fail → `Failed`, no panic; deliberate documented coupling, NOT decoupled); fixed-budget (4-draw) constant-time masked SM2 nonce sampler; zeroize sign nonce+intermediates (incl. `1+d`), CCM tentative-PT on tag-fail, `Sm3` `Drop`-wipes keyed state (makes the false `HmacSm3` zeroize-doc claim true at the field layer — do NOT `impl Drop for HmacSm3`); SM2 KDF `u32` wrap guard. **W3 (C ABI):** SM4-GCM/CCM/XTS FFI always-on in `gmcrypto-c` (drop the forwarding `sm4-aead`/`sm4-xts` cargo features) so `gmcrypto.h` == the default build; C shim default build now pulls `gmcrypto-simd`. **W4:** regenerated `cargo-public-api` baseline + docs. **Infra-assurance milestone, NOT a crates.io release** — breaking API/ABI ships with 1.0 (never a published 0.x crate); workspace stays 0.16.0, crates.io skips 0.23.0 (the v0.14/v0.17–v0.22 precedent). Verified byte-identical (KAT + gmssl 11/11 + full workspace + clippy/fmt/doc + 18 fuzz + MSRV + wasm32). codex+grok-reviewed W0–W4.
v1.0 = the deliberate first stable crates.io publish AFTER the §3.A decision (**resolved in v0.22** — crypto-bigint decoupled) AND the pre-publish re-audit (**remediated in v0.23**, merged #83). **Release-prep on `feat/v1.0-release`** (the mechanical, reversible half): bump 0.16.0→1.0.0 + exact `=1.0.0` sibling pins (`gmcrypto-core`→`gmcrypto-simd`, `gmcrypto-c`→`gmcrypto-core`); CHANGELOG `[1.0.0]` + README "1.0 stable" flip + `docs/v1.0-readiness.md`→GO + new `docs/v1.0.0-release-review.md`. crates.io jumps 0.16.0→1.0.0 (skips 0.17–0.23); only migration 0.16→1.0; runtime wire bytes byte-identical (gmssl 11/11). **Irreversible half = maintainer's post-merge step (NOT in the branch):** `cargo publish gmcrypto-simd → core → c`, SSH-signed `v1.0.0` tag + GitHub release, then flip `cargo-semver-checks` to enforced (`api-stability.yml`) — runbook `docs/v1.0-readiness.md` §4 steps 6–7. The merge + the publish are the user's calls.
v1.0.1 = readiness-cleanup patch release — the first post-1.0 crates.io publish (live; SSH-signed `v1.0.1` tag on the #92 merge commit + GitHub release). The GO-WITH-FOLLOWUP cleanup from the 2026-06-02 release-readiness synthesis (`docs/audits/2026-06-02-release-readiness-synthesis.md` — a dynamic-workflow synthesis of the 7 prior `docs/audits/2026-06-02-*` audits; 0 blockers, findings F1–F42). **Functional fix (the one behavior change, hence a publish not a docs update):** the `gmcrypto-c` C ABI `gmcrypto_version()` had returned a hardcoded `b"0.4.0"` regardless of the built version → now `concat!(env!("CARGO_PKG_VERSION"), "\0")`, reporting the real version (F3). **Doc:** raw-block "not a cipher mode" ECB warnings on `Sm4Cipher::{encrypt,decrypt}_block` + the block FFI (F30); cbindgen header pointer/length preconditions (F31); FFI RNG-fallible + XTS `start_sector`-range notes (F32/F34); `digest`/`cipher` pre-1.0-trait caveats (F26); `SECURITY.md`/`README.md`/`deny.toml` corrections (F1/F2/F11/F12/F16/F22–F25); `html_root_url` 0.5.0→1.0.0 (F4). **CI-health:** `sm4-xts` added to the MSRV/wasm32/`cargo deny` passes (F14); the dudect path-allowlist gained `gmcrypto-simd/src/**` (F15); `cargo generate-lockfile` before `cargo deny` (F19); a new `simd-x86` job (`cargo test -p gmcrypto-simd` on ubuntu-latest) that **caught a real latent bug** — the x86-only SIMD test files lacked `#![allow(unsafe_code)]` so they never compiled under CI's `-D warnings` (F17, fixed); and the removal of `pull_request` `paths-ignore` from `ci.yml` so docs-only PRs are no longer permanently blocked by branch-protection required checks (#91). **No API/ABI change; runtime crypto wire output byte-identical to 1.0.0** (#92 CI all-green incl. enforced `cargo-semver-checks` as the patch-non-breaking gate). 6 merged PRs (#87 doc/value/CI cleanup, #88 sm4-xts coverage + disclosure caveats, #89 simd-x86 job, #90 audit-trail commit, #91 doc-PR CI fix, #92 release). Workspace `version` 1.0.0→1.0.1; sibling pins `=1.0.0`→`=1.0.1`. The publish + tag + GitHub release were the user's (maintainer's) call; the agent path was branch + PR throughout.
post-1.0 / deferred = class-split-aware "noise-twin" dudect reference (the only design that could re-promote `ct_fn_invert`/`ct_fp_invert`) + round-trip/differential parser fuzzing + RustCrypto aead trait fit (blocked: aead still 0.6.0-rc.10) + AVX-512 sbox_x64 + CCM buffered input + the recurring `dudect-nightly` default+full leg-cancellation CI-health fix + (from the v1.0.1 synthesis, all non-blocking) a `ct_sm4_cbc_unpad` dudect target for the PKCS#7-strip path (F21 — needs a reviewed dudect re-baseline) + running the fuzz build on PRs vs the current nightly-only posture (F18) + gmssl interop in CI (F16). (NB: the §3.A crypto-bigint-exposure decision was **pre-1.0** and is now **resolved in v0.22** — see the `v0.22 =`/`v1.0 =` lines above; nothing pre-1.0 remains outstanding.)
