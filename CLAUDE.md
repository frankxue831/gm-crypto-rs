...
| | |
|---|---|
| Workspace version | `1.12.0` (sibling pins `=1.12.0`) — **prepped, not published**: publish simd → core → c, and tag, from **the tip of `main`** (not `9ccb262` — docs have landed since) is the maintainer's call. Gate #1 (`docs/ECOSYSTEM.md` §8) is **done** — PASS against `9ccb262`, `docs/v1.12.0-gate1-evidence.md`. It attaches to any tip for which `git diff 9ccb262 <tip> --stat -- crates/ Cargo.toml` is empty; check before tagging, and a PR touching `crates/` or `Cargo.toml` re-owes the gate. A new minor does not always mean crates.io (see Workflow) |
