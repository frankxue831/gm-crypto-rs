...
Pure-Rust SM2/SM3/SM4 SDK. **v0.23 — pre-1.0 re-audit remediation
...
v1.0 = the deliberate crates.io publish AFTER the §3.A decision (**resolved in v0.22** — crypto-bigint decoupled) AND the pre-publish re-audit (**remediated in v0.23**); bump 0.16.0→1.0.0 with exact `=1.0.0` sibling pins; publish `gmcrypto-simd → core → c`; flip `cargo-semver-checks` to enforced — runbook in `docs/v1.0-readiness.md` §4.
