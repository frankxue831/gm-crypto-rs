...
| `ct_sign_k_class` | nightly: `\|tau\| < 0.25`, PR-smoke: `\|tau\| < 0.35` | `sign_raw_with_id`, class-split by nonce `k` magnitude with `d` held fixed (W0; both retry nonces class-tied). v0.4 release-prep raised the gate per sample budget: the new GH Actions ubuntu-24.04 runner noise floor pushes this composite measurement to 0.16-0.20 at 100K samples and as high as 0.30 at 10K (variance ~3.16× wider; dudect `\|tau\|` scales with `1/sqrt(N)`). The underlying invert diagnostics (`ct_fn_invert` / `ct_fp_invert`) and the priv-key-class `ct_sign` target all measure well under 0.01 at 100K, so the regression-guard signal lives there. |
